...
export default class Game extends LightningElement {
  ...
  connectedCallback() {
    this.highScore = localStorage.getItem('lwc_snake_highscore') ? localStorage.getItem('lwc_snake_highscore') : 0;
  }
    ...
    if (this.tail.includes(`${this.xHead}:${this.yHead}`)) {
      ...
      if (this.gameBlocks[newPosIndex].food) {
        ...
        if (this.score > this.highScore) {
          ...
          localStorage.setItem('lwc_snake_highscore', this.highScore);
        }
        this.addSpeed();
        this.tail.push(`${this.xHead}:${this.yHead}`);
